...
I pool monthly IPUMS CPS ORG samples from 2003–2019 and treat the data as a repeated cross-section. 
...
Let $D_{st}$ be an indicator equal to one if state $s$ has a RTW law in force in year $t$ (defined below), and zero otherwise. For states that adopt RTW during the sample period, let $G_s$ denote the first year in which $D_{st}=1$; for never-adopting states I set $G_s=0$. 
...
I construct a state–month panel of RTW status and merge it to the CPS file by state and calendar month. 
The monthly RTW indicator equals one beginning in the month when a state's RTW statute becomes effective. 
For the empirical analysis, I aggregate this information to the annual level: $D_{st}=1$ if RTW is in force for at least six months of year $t$ in state $s$, and $D_{st}=0$ otherwise. 
...
In the event-study graphs I index time relative to adoption, using a symmetric window of five leads and five lags around the year before adoption (event time $-1$) as the omitted baseline.
...
I implement the Callaway and Sant'Anna (2021) difference-in-differences estimator for multiple periods and variation in treatment timing. 
Time is indexed by calendar year, the cohort variable is $G_s$, and I use the doubly-robust inverse-probability weighting estimator (\texttt{method(dripw)}). 
Observations are weighted by the CPS earnings weight and standard errors are clustered at the state level. 
For each outcome $Y_{ist}$ I report (i) event-study coefficients, which plot $\text{ATT}(g,t)$ aggregated across cohorts by event time, and (ii) average post-adoption effects that aggregate the cohort- and time-specific ATTs over post-treatment years.

\subsection{Baseline specification and controls}
In all specifications I condition on a common set of worker characteristics. 
...
I use the same control set for all outcomes (union coverage, union membership, free riding, and log hourly wages), and I always apply the CPS earnings weight and cluster standard errors at the state level, as described above.
...
Under the parallel-trends and overlap assumptions in \citet{CallawaySantAnna2021}, I can identify $\text{ATT}(g,t)$ by comparing workers in states that adopt RTW in year $g$ to workers in states that are not yet treated (plus never-treated states) in the same year.

I estimate \eqref{eq:att} using the doubly robust difference-in-differences estimator implemented in the \texttt{csdid} command, with calendar year as the time index and $G_s$ as the treatment cohort. 
...
To summarize the dynamics, I aggregate these cohort-specific effects by event time $k=t-g$ and report
\begin{equation}
\label{eq:event}
\delta_k
...
This approach is convenient in our setting because it delivers cohort–time average treatment effects that can be aggregated into overall post‑adoption effects and used consistently across the various outcomes I study, including RIF‑based measures of wage quantiles and subsamples defined by industry unionization. 
As a robustness check, I also estimate dynamic effects using the interaction‑weighted event‑study estimator of \citet{SunAbraham2021}, which likewise corrects the weighting problems of two‑way fixed‑effects models with staggered adoption. 

Appendix Figure A shows that the Sun–Abraham event‑study profiles are very similar to our baseline csdid estimates for unionization and wages, so I focus on the Callaway–Sant’Anna results in the main text.

%[[test]] 
\subsection{Distributional effects via RIF regressions}

Beyond average wage effects, I also study how RTW laws affect different parts of the wage distribution. 
Following \citet{FirpoSergio2009}, I use recentered influence function (RIF) regressions for unconditional quantiles of log hourly wages. For a given unconditional quantile $\tau$ of $\ln(\text{wage})$, I construct the RIF of log wages at $\tau$ and treat this transformed variable as the outcome in the same difference-in-differences design described above. 
The resulting coefficients can be interpreted as changes in the unconditional $\tau$th quantile of log wages.

Operationally, I estimate separate Callaway--Sant'Anna DID specifications with the RIF of log hourly wages as the dependent variable at several quantiles (e.g.\ the 10th, 25th, 50th, 75th, and 90th unconditional quantiles). 
I keep the same sample, weighting scheme, and set of controls as in the baseline mean-wage specification, and continue to cluster standard errors by state. 
In the results section, I summarize these estimates by plotting the ATT on log wages across quantiles, which shows how RTW affects the wage distribution rather than only the mean.%

\subsection{Heterogeneity by industry unionization}

I also examine whether RTW effects differ between more and less unionized industries. Using the pre-period CPS data, I compute industry-level union coverage rates (weighted by earnings weights) and define a high-union-industry indicator equal to one for industries with coverage above the median and zero otherwise. This industry measure is merged back to the full worker sample. To study heterogeneity, I re-estimate the baseline CSDID specifications separately for workers in high- and low-union industries, focusing on union outcomes and log wages. Comparing the resulting event-study profiles and average effects across these two subsamples allows us to assess whether RTW laws have stronger impacts in segments of the labor market where unions were initially more prevalent.
...
\input{Tables/table2_rtw_union_wage.tex}
}
\begin{tablenotes}
